...
public class SkyBlock(IHttpDeserializerService httpRequestAndDeserializer) : ISkyBlock
{
    public async Task<CollectionResponse> GetCollectionsAsync() =>
        await httpRequestAndDeserializer.RequestAndSerializeResponseAsync<CollectionResponse>("/recources/skyblock/collections");
    ...
    public async Task<ElectionsResponse> GetElectionsAsync() =>
        await httpRequestAndDeserializer.RequestAndSerializeResponseAsync<ElectionsResponse>("/resources/skyblock/election");
    ...
    public async Task<AuctionResponse> GetAuctionAsync(string input, InputType inputType) =>
        await httpRequestAndDeserializer.RequestAndSerializeResponseAsync<AuctionResponse>($"/skyblock/auction?{inputType}={input}");

    public async Task<AuctionsResponse> GetActiveAuctionsAsync(int page) =>
        await httpRequestAndDeserializer.RequestAndSerializeResponseAsync<AuctionsResponse>($"/skyblock/auctions?page={page}");

    public async Task<AuctionsHistoryResponse> GetAuctionsHistoryAsync() =>
        await httpRequestAndDeserializer.RequestAndSerializeResponseAsync<AuctionsHistoryResponse>("/skyblock/auctions_ended");
    ...
    public async Task<ProfilesResponse> GetProfilesAsync(string uuid) => await httpRequestAndDeserializer.RequestAndSerializeResponseAsync<ProfilesResponse>("/skyblock/profiles");

    public async Task<BingoCompletionsResponse> GetBingoCompletionsAsync(string uuid) =>
        await httpRequestAndDeserializer.RequestAndSerializeResponseAsync<BingoCompletionsResponse>($"/skyblock/bingo?uuid={uuid}");
}